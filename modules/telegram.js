...
async function cmdWebApp(chatId, user) {
  ...
  if (role === 'client') {
    ...
    url = baseUrl + '/app-worker?v=' + Date.now() + cryptoR.randomUUID().slice(0, 8);
...
async function handleTgMessage(body) {
  ...
  if (text === '/start' || text.startsWith('/start@')) {
    ...
    if (existingUser) {
      const roleNames = { owner: '👑 Владелец', dispatcher: '📋 Диспетчер', worker: '👷 Исполнитель', client: '🏢 Клиент' };
            const waUrl = (config.webAppUrl || 'https://диспетчер-про.рф') + "/app-worker?v=" + Date.now() + require('crypto').randomUUID().slice(0, 8);
