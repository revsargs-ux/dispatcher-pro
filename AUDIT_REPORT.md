...
| 3 | `modules/config.js` | Захардкожен Supabase URL (`https://bzozrjgfnpdhlymfuobd.supabase.co`) — утечка в открытый код |
...
### Env Variables
- **`SB_URL`** — теперь ОБЯЗАТЕЛЬНА (убран хардкод). Убедитесь что задана в окружении: `https://bzozrjgfnpdhlymfuobd.supabase.co`
