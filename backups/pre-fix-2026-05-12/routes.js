...
const { createToken, requireAuth, hashPassword, checkPassword, isPlaintext, checkRateLimit, resetRateLimit } = require('./auth');
...
async function handleTelegramStatus(req, res, cors) {
  ...
  try {
    ...
    const rows = await (await sbFetch(table, `${phoneCol}=ilike.%25${phone.slice(-10)}%25&select=telegram_chat_id&limit=1`)).json();
    json(res, { linked: rows.length > 0 && !!rows[0].telegram_chat_id }, 200, cors);
...
async function handleLogin(req, res, cors) {
  ...
  try {
    ...
    const phoneCol = table === 'clients' ? 'contact' : 'phone';
    const users = await (await sbFetch(table, `${phoneCol}=ilike.%25${cleanPhone.slice(-10)}%25&limit=1`)).json();
...
async function handleRegister(req, res, cors) {
  ...
  try {
    ...
    if (!regSession) {
      ...
      if (table === 'users' && data.role && data.role !== 'dispatcher') return json(res, { ok: false, error: 'Саморегистрация доступна только для диспетчеров' }, 403, cors);
    } else if (!['owner', 'dispatcher'].includes(regSession.role)) {
      return json(res, { ok: false, error: 'Нет прав для регистрации' }, 403, cors);
    }
...
// --- Telegram webhook ---
async function handleTgWebhookRoute(req, res) {
  const body = await readBody(req);
  try { handleTgMessage(JSON.parse(body)); } catch (e) { console.error('[TG] Webhook parse error:', e.message); }
...
function createRouter() {
  return async (req, res) => {
    ...
    if (urlPath === '/tg/webhook' && req.method === 'POST') return await handleTgWebhookRoute(req, res);
